...
class ContentFragmentViewModel(application: Application) : AndroidViewModel(application) {
     ...
     * UI state
     */
    val uiState: StateFlow<ContentUiState> field = MutableStateFlow<ContentUiState>(ContentUiState.Loading)
    ...
    init {
        ...
        viewModelScope.launch {
            ...
            uiState.emit(ContentUiState.Loading)
            application.sessionManager.session.collect { session ->
                when (session) {
                    is Session.Active -> {
                        Napier.d { "Has active user: ${session.user.username}" }
                        uiState.emit(ContentUiState.Content(session.user.username, true))
                    }
                    Session.NotLoggedIn -> {
